# GitOps Templates -- Todo

## Priority: High
- [x] Create base Helm chart structure (`charts/app-base/Chart.yaml`, `values.yaml`, `_helpers.tpl`) -- PR #1
- [x] Write Deployment template (`charts/app-base/templates/deployment.yaml`) -- PR #2
- [ ] Write Service template (`charts/app-base/templates/service.yaml`)
- [x] Write Ingress template (`charts/app-base/templates/ingress.yaml`) -- PR #3
- [ ] Write HPA template (`charts/app-base/templates/hpa.yaml`)
- [ ] Write ConfigMap template (`charts/app-base/templates/configmap.yaml`)
- [ ] Write ServiceAccount template (`charts/app-base/templates/serviceaccount.yaml`)
- [ ] Create `values.schema.json` for the base chart
- [ ] Add GitHub Actions lint workflow (`.github/workflows/lint.yml`)

## Priority: Medium
- [ ] Create example consumer values file (`examples/service-values.yaml`)
- [ ] Add kubeval/kubeconform validation to CI workflow
- [ ] Create Kustomize base as alternative to Helm (`kustomize/base/`)

## Priority: Low
- [ ] Add chart versioning and release workflow
- [ ] Create additional chart variants (CronJob, StatefulSet)
- [ ] Add Helm chart repository index for proper `helm repo add` support

## Completed
<!-- [x] Task description -- PR #N -->
